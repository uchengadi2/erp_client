...
import { EDIT_PROJECTPROJECT } from "../../../actions/types";
...
function ProjectsProjectsProjectsEditForm(props) {
  const { params } = props;
  const classes = useStyles();
  const [projectManager, setProjectManager] = useState(params.projectManager);
  ...
  const buttonContent = () => {
    return <React.Fragment> Update Project</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const editForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.patch(`/projects/${params.id}`, formValues);
  ...
  const projectStartDate = params.startDate
    ? new Date(params.startDate).toISOString().slice(0, 10)
    : "";

  const projectEndDate = params.endDate
    ? new Date(params.endDate).toISOString().slice(0, 10)
    : "";

  return (
    <form id="projectsProjectsProjectsEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
        ...
        <Grid container="row">
          <Grid item style={{ width: "45%" }}>
            <Field
              ...
              id="totalBudget"
              name="totalBudget"
              defaultValue={params.totalBudget}
              type="number"
          ...
          <Grid item style={{ width: "52%", marginLeft: 10 }}>
            <Field
              ...
              defaultValue={params.remainingBudgetedAmount}
              type="number"
...
export default reduxForm({
  form: "projectsProjectsProjectsEditForm",
