...
import { CREATE_PROJECTPROJECTRESOURCE } from "../../../../actions/types";
...
function ProjectsProjectsResourceCreateForm(props) {
  const classes = useStyles();
  const [assetType, setAssetType] = useState();
  const [assetSet, setAssetSet] = useState();
  const [assetSetBatch, setAssetSetBatch] = useState();
  ...
  console.log("thavailable stock quantity:", availableStockQuantity);
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Resource</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "PJ" + "-" + Math.floor(Math.random() * 1000000) + "-" + "RSO";
    }

    if (
      parseFloat(availableStockQuantity) <
      parseFloat(formValues["stockQuantity"])
    ) {
      formValues["stockQuantity"] = parseFloat(availableStockQuantity);
    }

    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/projectresources`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.refNumber}-${response.data.data.data.label} Resource is added successfully!!!`
          );
  ...
  return (
    <form id="projectsProjectsResourceCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "projectsProjectsResourceCreateForm",
