...
import { EDIT_PROJECTPLANNINGTASK } from "../../../../actions/types";
...
function ProjectsPlanningTasksEditForm(props) {
  const { params } = props;
  ...
  const buttonContent = () => {
    return <React.Fragment> Update Task</React.Fragment>;
  };
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/projectplanningtasks/${params.id}`,
          formValues
        );
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.task} Task is updated successfully!!!`
          );
  ...
  const taskStartDate = params.startDate
    ? new Date(params.startDate).toISOString().slice(0, 10)
    : "";

  const taskEndDate = params.endDate
    ? new Date(params.endDate).toISOString().slice(0, 10)
    : "";

  return (
    <form id="projectsPlanningTasksEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
      ...
      >
        ...
        >
          <FormLabel
            ...
            component="legend"
          >
            <Typography variant="subtitle1"> Update Task</Typography>
          </FormLabel>
        </Grid>
        <Field
          label=""
...
export default reduxForm({
  form: "projectsPlanningTasksEditForm",
