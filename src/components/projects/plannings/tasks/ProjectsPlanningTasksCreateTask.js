...
import { CREATE_PROJECTPLANNINGTASK } from "../../../../actions/types";
...
function ProjectsPlanningTasksCreateTask(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Task</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "PR" + "-" + Math.floor(Math.random() * 1000000) + "-" + "TAK";
    }

    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/projectplanningtasks`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.task} Task is aadded successfully!!!`
          );
  ...
  return (
    <form id="projectsPlanningTasksCreateTask">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "projectsPlanningTasksCreateTask",
})(ProjectsPlanningTasksCreateTask);
