...
import { EDIT_PROJECTCLOSURE } from "../../../../actions/types";
...
function ProjectsClosureProjectEditForm(props) {
  const { params } = props;
  const classes = useStyles();
  const [project, setProject] = useState(params.project);
  const [task, setTask] = useState();
  const [user, setUser] = useState(params.closedBy);
  ...
  const closedDate = params.closedDate
    ? new Date(params.dateClosed).toISOString().slice(0, 10)
    : "";

  return (
    <form id="projectsClosureProjectEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "projectsClosureProjectEditForm",
