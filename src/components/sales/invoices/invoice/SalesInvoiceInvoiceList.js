...
import SalesInvoiceInvoiceDelete from "./SalesInvoiceInvoiceDelete";
import SalesInvoiceInvoiceEditForm from "./SalesInvoiceInvoiceEditForm";

class SalesInvoiceInvoiceList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editOpen: false,
  ...
  componentDidMount() {
    this.props.fetchSalesInvoices(this.props.token);
  }
  ...
  renderDataList = () => {
          ...
          <strong>
            ...
            <EditRoundedIcon
              onClick={() => [
                ...
                history.push(`/sales/invoices/invoices/edit/${params.id}`),
              ]}
            />
          </strong>
        ),
          ...
          <strong>
            ...
            <DeleteRoundedIcon
              ...
              onClick={() => [
                ...
                history.push(`/sales/invoices/invoices/delete/${params.id}`),
              ]}
            />
          </strong>
        ),
    ...
    this.props.salesInvoices.map((salesInvoice) => {
      let row = {
        numbering: ++counter,
        id: salesInvoice.id,
...
const mapStateToProps = (state) => {
  return { salesInvoices: Object.values(state.salesInvoice) };
};

export default connect(mapStateToProps, { fetchSalesInvoices })(
  SalesInvoiceInvoiceList
);
