...
import { CREATE_ASSETPROCUREMENT } from "../../../../actions/types";
...
function AssetExecutedProcurementCreateForm(props) {
  ...
  const [procuredBy, setProcuredBy] = useState();
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Procurement</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    formValues["procurementRefNumber"] = Math.floor(Math.random() * 100000000);
    formValues["remainingQuantityForAllocation"] = formValues["quantity"];

    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/assetprocurements`, formValues);
  ...
  return (
    <form id="assetExecutedProcurementCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetExecutedProcurementCreateForm",
})(AssetExecutedProcurementCreateForm);
