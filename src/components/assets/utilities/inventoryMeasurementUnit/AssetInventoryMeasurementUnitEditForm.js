...
import { EDIT_ASSETINVENTORYMEASUREMENTUNIT } from "../../../../actions/types";
...
function AssetInventoryMeasurementUnitEditForm(props) {
  const { params } = props;
  const classes = useStyles();
  const [assetInventoryType, setAssetInventoryType] = useState(
    params.assetInventoryType
  );
  ...
  const buttonContent = () => {
    return <React.Fragment>Update Inventory Measurement Unit</React.Fragment>;
  };
    ...
    if (formValues) {
      const createForm = async () => {
        ...
        const response = await api.patch(
          `/assetinventorymeasurementunits/${params.id}`,
          formValues
        );
  ...
  return (
    <form id="assetInventoryMeasurementUnitEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetInventoryMeasurementUnitEditForm",
