...
import { fetchMeasurementUnits } from "../../../../actions";
import DataGridContainer from "../../../DataGridContainer";

import AssetUtilityMeasurementUnitDelete from "./AssetUtilityMeasurementUnitDelete";
import AssetUtilityMeasurementUnitEditForm from "./AssetUtilityMeasurementUnitEditForm";

class AssetUtilityMeasurementUnitList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editOpen: false,
  ...
  componentDidMount() {
    this.props.fetchMeasurementUnits(this.props.token);
  }
  ...
  renderDeleteDialogForm = () => {
    ...
    return (
      <>
        ...
        >
          <DialogContent>
            <AssetUtilityMeasurementUnitDelete
              token={this.props.token}
              userId={this.props.userId}
              id={this.state.id}
  ...
  renderDepreciationTypesList = () => {
          ...
          <strong>
            ...
            <EditRoundedIcon
              onClick={() => [
                ...
                history.push(
                  `/assets/utilities/measurementunits/edit/${params.id}`
                ),
              ]}
            />
          </strong>
          ...
          <strong>
            ...
            <DeleteRoundedIcon
              ...
              onClick={() => [
                ...
                history.push(
                  `/assets/utilities/measurementunits/delete/${params.id}`
                ),
              ]}
            />
          </strong>
    ...
    this.props.measurementUnits.map((measurementUnit) => {
      let row = {
        numbering: ++counter,
...
const mapStateToProps = (state) => {
  return { measurementUnits: Object.values(state.measurementUnit) };
};

export default connect(mapStateToProps, { fetchMeasurementUnits })(
