...
import { CREATE_ASSETINVENTORY } from "../../../../actions/types";
...
function AssetInventoryCreateForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Inventory</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!serviceOutlet) {
      props.handleFailedSnackbar("Please select the service outlet");
      setLoading(false);
      return;
    }
    if (!inventoryType) {
      props.handleFailedSnackbar("Please select the inventory type");
      setLoading(false);
      return;
    }

    if (!formValues["name"]) {
      props.handleFailedSnackbar("Please enter the name of the inventory");
      setLoading(false);
      return;
    }

    if (!storeType) {
      props.handleFailedSnackbar("Please select the store type");
      setLoading(false);
      return;
    }

    if (!store) {
      props.handleFailedSnackbar("Please select the store");
      setLoading(false);
      return;
    }

    if (!formValues["quantity"]) {
      props.handleFailedSnackbar("Please enter the inventory quantity");
      setLoading(false);
      return;
    }

    if (!inventoryMeasurementUnit) {
      props.handleFailedSnackbar(
        "Please select the inventory measurement unit"
      );
      setLoading(false);
      return;
    }

    if (!formValues["datePlacedInStore"]) {
      props.handleFailedSnackbar("Please enter the date placed in store");
      setLoading(false);
      return;
    }

    if (!capacityUnit) {
      props.handleFailedSnackbar("Please select the Inventory Capacity unit");
      setLoading(false);
      return;
    }

    if (!formValues["totalCapacity"]) {
      props.handleFailedSnackbar("Please enter the total inventory capacity");
      setLoading(false);
      return;
    }

    if (!formValues["totalInventoryCost"]) {
      props.handleFailedSnackbar("Please enter the total inventory cost");
      setLoading(false);
      return;
    }

    if (!currency) {
      props.handleFailedSnackbar("Please select the currency");
      setLoading(false);
      return;
    }

    if (!formValues["costPerUnit"]) {
      props.handleFailedSnackbar("Please enter the cost per unit");
      setLoading(false);
      return;
    }

    if (!status) {
      props.handleFailedSnackbar("Please select the inventory status");
      setLoading(false);
      return;
    }
    ...
    formValues["remainingCapacity"] = formValues["totalCapacity"];

    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "INV" + "-" + Math.floor(Math.random() * 100000000000) + "-" + "AS";
    }

    if (!formValues["sku"]) {
      formValues["sku"] =
        "SKU" + "-" + Math.floor(Math.random() * 100000000000) + "-" + "INV";
    }

    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/assetinventories`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.refNumber} Inventory is added successfully!!!`
          );
  ...
  return (
    <form id="assetInventoryCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
