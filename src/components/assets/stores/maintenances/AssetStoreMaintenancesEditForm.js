...
import { EDIT_MAINTENANCEASSETSTORE } from "../../../../actions/types";
...
function AssetStoreMaintenancesEditForm(props) {
  ...
  const [storeMaintenanceType, setStoreMaintenanceType] = useState(
    props.params.storeMaintenanceType
  );
  ...
  const buttonContent = () => {
    return <React.Fragment> Update Store Maintenance</React.Fragment>;
  };
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/storemaintenances/${props.params.id}`,
          formValues
        );
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.label} Store Maintenance is updated successfully!!!`
          );
  ...
  const commencementDate = new Date(params.maintenanceCommencementDate)
    .toISOString()
    .slice(0, 10);

  const endDate = new Date(params.maintenanceEndDate)
    .toISOString()
    .slice(0, 10);

  return (
    <form id="assetStoreMaintenancesEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetStoreMaintenancesEditForm",
