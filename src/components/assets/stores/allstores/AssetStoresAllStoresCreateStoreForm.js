...
import { CREATE_ASSETSTORE } from "../../../../actions/types";
...
function AssetStoresAllStoresCreateStoreForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Create Store</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    const unallocatedSpace = +(
      (+formValues["length"] - +formValues["unuseableRegionLength"]) *
      (+formValues["breadth"] - +formValues["unuseableRegionBreadth"])
    );
    formValues["totalAllocatableSpace"] = unallocatedSpace;
    formValues["unallocatedSpace"] = unallocatedSpace;
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/stores`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.name} Store is created successfully!!!`
          );
  ...
  return (
    <form id="assetStoresAllStoresCreateStoreForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetStoresAllStoresCreateStoreForm",
})(AssetStoresAllStoresCreateStoreForm);
