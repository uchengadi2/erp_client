...
import { CREATE_LEASEASSETSTORE } from "../../../../actions/types";
...
function AssetStoreLeasesCreateForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Store Lease</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/storeleases`, formValues);
  ...
  return (
    <form id="assetStoreLeasesCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetStoreLeasesCreateForm",
