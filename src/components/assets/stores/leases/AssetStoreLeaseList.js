...
import { fetchLeaseAssetStores } from "../../../../actions";
import DataGridContainer from "../../../DataGridContainer";

import AssetStoreLeaseDelete from "./AssetStoreLeaseDelete";
import AssetStoreLeaseEditForm from "./AssetStoreLeaseEditForm";

class AssetStoreLeaseList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editOpen: false,
  ...
  componentDidMount() {
    this.props.fetchLeaseAssetStores(this.props.token);
  }
    ...
    return (
      <>
        <Dialog
          ...
          onClose={() => [
            this.setState({ editOpen: false }),
            history.push("/assets/stores/leases"),
          ]}
        >
          <DialogContent>
  ...
  renderAssetStoreLeasesList = () => {
          ...
          <strong>
            ...
            <EditRoundedIcon
              onClick={() => [
                ...
                history.push(`/assets/stores/leases/edit/${params.id}`),
              ]}
            />
          </strong>
        ),
          ...
          <strong>
            ...
            <DeleteRoundedIcon
              ...
              onClick={() => [
                ...
                history.push(`/assets/stores/leases/delete/${params.id}`),
              ]}
            />
          </strong>
        ),
    ...
    this.props.assetStoreLeases.map((assetStoreLease) => {
      let row = {
        numbering: ++counter,
  ...
  render() {
    return (
      <>
        ...
        {this.renderAssetStoreLeasesList()}
        <Snackbar
...
const mapStateToProps = (state) => {
  return { assetStoreLeases: Object.values(state.assetStoreLease) };
};

export default connect(mapStateToProps, { fetchLeaseAssetStores })(
  AssetStoreLeaseList
);
