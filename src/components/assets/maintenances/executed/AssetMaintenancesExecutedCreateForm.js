...
import { CREATE_EXECUTEDMAINTENANCE } from "../../../../actions/types";
...
function AssetMaintenancesExecutedCreateForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/assetmaintenances`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.maintenanceRefNumber} Stock Maintenance is added successfully!!!`
          );
  ...
  return (
    <form id="assetMaintenancesExecutedCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "assetMaintenancesExecutedCreateForm",
