...
import { EDIT_ASSETDISPOSITION } from "../../../../actions/types";
...
function AssetDispositionsAllDispositionEditForm(props) {
  const { params } = props;
  ...
  const onSubmit = (formValues) => {
    ...
    //     "DS" + "-" + Math.floor(Math.random() * 1000000) + "-" + "ST";
    // }
      ...
      const editForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.dispositionRefNumber} Stock Disposition is updated successfully!!!`
          );
  ...
  const dateOfDisposition = new Date(params.dispositionDate)
    .toISOString()
    .slice(0, 10);
  return (
    <form id="assetDispositionsAllDispositionEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
        ...
        <Grid container direction="row" style={{ marginTop: 10 }}>
          <Grid item>
            <Field
              ...
              defaultValue={params.dispositionRefNumber}
              type="text"
        ...
        <Grid container direction="row" style={{ marginTop: 10 }}>
          <Grid item>
            <Field
              ...
              defaultValue={dateOfDisposition}
              type="date"
          ...
          <Grid item>
            <Field
              ...
              defaultValue={params.totalDispositionCost}
              type="number"
...
export default reduxForm({
  form: "assetDispositionsAllDispositionEditForm",
