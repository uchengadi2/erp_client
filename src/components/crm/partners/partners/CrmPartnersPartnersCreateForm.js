...
import { CREATE_PARTNER } from "../../../../actions/types";
...
function CrmPartnersPartnersCreateForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    const data = {
      partnerNumber: formValues["partnerNumber"]
        ...
        : "PIS-" + Math.floor(Math.random() * 100000000),
    ...
    if (data) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/crmpartners`, data);
  ...
  return (
    <div className={classes.root}>
      <form id="crmPartnersPartnersCreateForm" className={classes.formStyles}>
...
export default reduxForm({
  form: "crmPartnersPartnersCreateForm",
