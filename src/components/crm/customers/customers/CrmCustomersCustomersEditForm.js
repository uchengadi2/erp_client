...
import { EDIT_CUSTOMER } from "../../../../actions/types";
...
function CrmCustomersCustomersEditForm(props) {
  const { params } = props;
  ...
  const [customerType, setCustomerType] = useState(params.type);
  ...
  const onSubmit = (formValues) => {
    ...
    if (data) {
      const editForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.patch(`/crmcustomers/${params.id}`, data);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.name} Customer is updated successfully!!!`
          );
  ...
  return (
    <div className={classes.root}>
      <form id="crmCustomersCustomersEditForm" className={classes.formStyles}>
...
export default reduxForm({
  form: "crmCustomersCustomersEditForm",
