...
function CrmSuppliersSuppliersCreateForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    const data = {
      supplierNumber: formValues["supplierNumber"]
        ...
        : "SIS-" + Math.floor(Math.random() * 100000000),
    ...
    if (data) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/crmsuppliers`, data);
  ...
  return (
    <div className={classes.root}>
      <form id="crmSuppliersSuppliersCreateForm" className={classes.formStyles}>
...
export default reduxForm({
  form: "crmSuppliersSuppliersCreateForm",
})(CrmSuppliersSuppliersCreateForm);
