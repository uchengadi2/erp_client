...
const renderNameField = ({
  ...
  return (
    <TextField
      ...
      helperText="Enter the name of the Supplier"
      variant="outlined"
      label={label}
      id={input.name}
      fullWidth
...
const renderVendorAddressField = ({
  ...
  return (
    <TextField
      //error={touched && invalid}
      helperText="Enter the Supplier Address Location"
      variant="outlined"
      label={label}
      id={input.name}
      fullWidth
...
function CrmSuppliersSuppliersEditForm(props) {
  const { params } = props;
  ...
  const [supplierType, setSupplierType] = useState(params.type);
  ...
  const onSubmit = (formValues) => {
    ...
    const data = {
      ...
      products: formValues["products"]
        ...
        : params.products,
      type: supplierType,
      supplierCountry: locationCountry,
      location: {
    ...
    if (data) {
      const editForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.patch(`/crmsuppliers/${params.id}`, data);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.name} Supplier is updated successfully!!!`
          );
  ...
  return (
    <div className={classes.root}>
      <form id="crmSuppliersSuppliersEditForm" className={classes.formStyles}>
...
export default reduxForm({
  form: "crmSuppliersSuppliersEditForm",
