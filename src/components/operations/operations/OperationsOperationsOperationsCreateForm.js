...
import { CREATE_OPERATIONOPERATION } from "../../../actions/types";
...
function OperationsOperationsOperationsCreateForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Start Operation</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (formValues) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/operationoperations`, formValues);
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.refNumber}-${response.data.data.data.label} Operation is initiated successfully!!!`
          );
  ...
  return (
    <form id="operationsOperationsOperationsCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
      ...
      >
        ...
        >
          <FormLabel
            ...
            component="legend"
          >
            <Typography variant="subtitle1"> Initiate Operation</Typography>
          </FormLabel>
        </Grid>
        <Field
          label=""
...
export default reduxForm({
  form: "operationsOperationsOperationsCreateForm",
})(OperationsOperationsOperationsCreateForm);
