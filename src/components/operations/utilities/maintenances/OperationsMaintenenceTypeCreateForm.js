...
import { CREATE_OPERATIONMAINTENANCETYPE } from "../../../../actions/types";
...
function OperationsMaintenenceTypeCreateForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Maintenance Type</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "OP" + "-" + Math.floor(Math.random() * 1000000) + "-" + "MTY";
    }
      ...
      const createForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.name} Maintenance Type is added successfully!!!`
          );
  ...
  return (
    <form id="operationsMaintenenceTypeCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "operationsMaintenenceTypeCreateForm",
})(OperationsMaintenenceTypeCreateForm);
