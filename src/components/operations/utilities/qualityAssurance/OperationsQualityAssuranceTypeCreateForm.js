...
import { CREATE_QUALITYASSURANCETYPE } from "../../../../actions/types";
...
const renderNameField = ({
  ...
  return (
    <TextField
      ...
      helperText="Enter Quality Assurance Type"
      variant="outlined"
      label={label}
      id={input.name}
...
const renderDescriptionField = ({
  ...
  return (
    <TextField
      ...
      variant="outlined"
      helperText="Describe Quality Assurance Type"
      label={label}
      id={input.name}
      name={input.name}
      defaultValue={input.value}
...
function OperationsQualityAssuranceTypeCreateForm(props) {
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Quality Assurance Type</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "OP" + "-" + Math.floor(Math.random() * 1000000) + "-" + "QAT";
    }
      ...
      const createForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.name} Quality Assurance Type is added successfully!!!`
          );
  ...
  return (
    <form id="operationsQualityAssuranceTypeCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
...
export default reduxForm({
  form: "operationsQualityAssuranceTypeCreateForm",
