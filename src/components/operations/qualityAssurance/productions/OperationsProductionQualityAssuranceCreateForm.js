...
import { CREATE_PRODUCTIONQUALITYASSURANCE } from "../../../../actions/types";
...
function OperationsProductionQualityAssuranceCreateForm(props) {
  ...
  const renderExtraQualityAssuranceCostCurrencyField = ({
    input,
    label,
    meta: { touched, error, invalid },
    type,
  ...
  const buttonContent = () => {
    return <React.Fragment> Add Quality Assurance</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    if (!maintenanceType) {
      props.handleFailedSnackbar(
        "Please select the Maintenance Type and try again"
      );
      setLoading(false);
      return;
    }
    ...
    if (!formValues["refNumber"]) {
      formValues["refNumber"] =
        "OP" + "-" + Math.floor(Math.random() * 100000000000) + "-" + "QAS";
    }
      ...
      const createForm = async () => {
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulCreateSnackbar(
            `${response.data.data.data.refNumber}-${response.data.data.data.label} Quality Assurance is added successfully!!!`
          );
  ...
  return (
    <form id="operationsProductionQualityAssuranceCreateForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
        ...
        <Grid container="row">
          ...
          <Grid item style={{ width: "52%", marginLeft: 10 }}>
            <Field
              ...
              type="text"
              component={renderExtraQualityAssuranceCostCurrencyField}
              style={{ marginTop: 5 }}
            />
          </Grid>
        </Grid>
...
export default reduxForm({
  form: "operationsProductionQualityAssuranceCreateForm",
})(OperationsProductionQualityAssuranceCreateForm);
