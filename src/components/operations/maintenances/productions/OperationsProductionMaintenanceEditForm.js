...
import { EDIT_OPERATIONPRODUCTIONMAINTENANCE } from "../../../../actions/types";
...
const renderMaintenanceCostField = ({
  ...
  return (
    <TextField
      //error={touched && invalid}
      helperText="Extra Maintenance Cost"
      variant="outlined"
      label={label}
      id={input.name}
      defaultValue={0}
...
function OperationsProductionMaintenanceEditForm(props) {
  const { params } = props;
  ...
  const buttonContent = () => {
    return <React.Fragment>Update Maintenance</React.Fragment>;
  };
  ...
  const onSubmit = (formValues) => {
    ...
    formValues["extraMaintenanceCostCurrency"] = currency;
    ...
    if (formValues) {
      const createForm = async () => {
        ...
        const response = await api.patch(
          `/operationproductionmaintenances/${params.id}`,
          formValues
        );
        ...
        if (response.data.status === "success") {
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.refNumber}-${response.data.data.data.label} Maintenance is updated successfully!!!`
          );
  ...
  const dateOfMaintenance = params.maintenanceDate
    ? new Date(params.maintenanceDate).toISOString().slice(0, 10)
    : "";

  return (
    <form id="operationsProductionMaintenanceEditForm">
      <Box
        // component="form"
        // id="categoryForm"
        // onSubmit={onSubmit}
      ...
      >
        ...
        <Grid container="row">
          <Grid item style={{ width: "45%", marginLeft: 0 }}>
            <Field
              label=""
              id="extraMaintenanceCost"
              name="extraMaintenanceCost"
              defaultValue={params.extraMaintenanceCost}
              type="number"
          ...
          <Grid item style={{ width: "52%", marginLeft: 10 }}>
            <Field
              label=""
              id="extraMaintenanceCostCurrency"
              name="extraMaintenanceCostCurrency"
              defaultValue={params.extraMaintenanceCostCurrency}
              type="text"
...
export default reduxForm({
  form: "operationsProductionMaintenanceEditForm",
