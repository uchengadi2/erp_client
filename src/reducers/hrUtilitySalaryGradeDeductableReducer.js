import _ from "lodash";
import {
  CREATE_HRUTILITYSALARYGRADEDEDUCTABLE,
  FETCH_HRUTILITYSALARYGRADEDEDUCTABLES,
  FETCH_HRUTILITYSALARYGRADEDEDUCTABLE,
  DELETE_HRUTILITYSALARYGRADEDEDUCTABLE,
  EDIT_HRUTILITYSALARYGRADEDEDUCTABLE,
} from "./../actions/types";

// const INITIALSTATE = {
//   status: null,
//   data: {},
// };

export default (state = {}, action) => {
  switch (action.type) {
    case FETCH_HRUTILITYSALARYGRADEDEDUCTABLES:
      let id = 0;
      return {
        ...state,
        ..._.mapKeys(action.payload, `id`),
      };

    case FETCH_HRUTILITYSALARYGRADEDEDUCTABLE:
      return { ...state, [action.payload.id]: action.payload };
    case CREATE_HRUTILITYSALARYGRADEDEDUCTABLE:
      return { ...state, [action.payload.id]: action.payload };
    case EDIT_HRUTILITYSALARYGRADEDEDUCTABLE:
      return { ...state, [action.payload.id]: action.payload };
    case DELETE_HRUTILITYSALARYGRADEDEDUCTABLE:
      return _.omit(state, action.payload); //note that payload is just the state id
    default:
      return state;
  }
};
