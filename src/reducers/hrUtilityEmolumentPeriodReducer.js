import _ from "lodash";
import {
  CREATE_HRUTILITYEMOLUMENTPERIOD,
  FETCH_HRUTILITYEMOLUMENTPERIODS,
  FETCH_HRUTILITYEMOLUMENTPERIOD,
  DELETE_HRUTILITYEMOLUMENTPERIOD,
  EDIT_HRUTILITYEMOLUMENTPERIOD,
} from "./../actions/types";

// const INITIALSTATE = {
//   status: null,
//   data: {},
// };

export default (state = {}, action) => {
  switch (action.type) {
    case FETCH_HRUTILITYEMOLUMENTPERIODS:
      let id = 0;
      return {
        ...state,
        ..._.mapKeys(action.payload, `id`),
      };

    case FETCH_HRUTILITYEMOLUMENTPERIOD:
      return { ...state, [action.payload.id]: action.payload };
    case CREATE_HRUTILITYEMOLUMENTPERIOD:
      return { ...state, [action.payload.id]: action.payload };
    case EDIT_HRUTILITYEMOLUMENTPERIOD:
      return { ...state, [action.payload.id]: action.payload };
    case DELETE_HRUTILITYEMOLUMENTPERIOD:
      return _.omit(state, action.payload); //note that payload is just the state id
    default:
      return state;
  }
};
